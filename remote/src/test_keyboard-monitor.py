...
KEY_NAMES = {
    ...
    pygame.K_SEMICOLON: ";", pygame.K_QUOTE: "'", pygame.K_LEFTBRACKET: "[", pygame.K_RIGHTBRACKET: "]",
    pygame.K_BACKSLASH: "\\", pygame.K_EQUALS: "=", pygame.K_MINUS: "-",
}
...
KEYBOARD_LAYOUT = [
    [
        ...
        None, None, None, None,
        (pygame.K_F1, "F1", 12), (pygame.K_F2, "F2", 12), (pygame.K_F3, "F3", 12), (pygame.K_F4, "F4", 12),
        None,
        (pygame.K_F5, "F5", 12), (pygame.K_F6, "F6", 12), (pygame.K_F7, "F7", 12), (pygame.K_F8, "F8", 12),
        None,
        (pygame.K_F9, "F9", 12), (pygame.K_F10, "F10", 12), (pygame.K_F11, "F11", 12), (pygame.K_F12, "F12", 12),
    ],
    [
    ...
    [
        ...
        (pygame.K_LEFTBRACKET, "[", 15), (pygame.K_RIGHTBRACKET, "]", 15), (pygame.K_BACKSLASH, "\\", 22),
    ],
    [
        ...
        (pygame.K_k, "K", 15), (pygame.K_l, "L", 15), (pygame.K_SEMICOLON, ";", 15), 
        (pygame.K_QUOTE, "'", 15), (pygame.K_RETURN, "ENTER", 32),
    ],
    [
        ...
        (pygame.K_COMMA, ",", 15), (pygame.K_PERIOD, ".", 15), (pygame.K_SLASH, "/", 15), 
        (pygame.K_RSHIFT, "SHIFT", 30), None, (pygame.K_UP, "UP", 15),
    ],
    [
        (pygame.K_LCTRL, "CTRL", 25), (pygame.K_LALT, "ALT", 25), (pygame.K_SPACE, "SPACE", 107), 
...
def draw_keyboard(surface, pressed_keys_set):
    """キーボード レイアウトを描画"""
    ...
    for row_idx, row in enumerate(KEYBOARD_LAYOUT):
        ...
        for item in row:
            ...
            is_pressed = key_code in pressed_keys_set
            
            # キーの背景色
            bg_color = COLOR_KEY_PRESSED if is_pressed else COLOR_KEY_NORMAL
            
            # キーを描画
            pygame.draw.rect(surface, bg_color, (x, y, width, 15), border_radius=2)
            pygame.draw.rect(surface, (200, 200, 200), (x, y, width, 15), 1, border_radius=2)
...
def main():
    ...
    # ディスプレイを作成
    screen = pygame.display.set_mode((300, 115))
    pygame.display.set_caption("Keyboard Monitor")
    ...
    last_pressed_keys = []
    running = True
    pressed_keys_set = set()  # 現在押下されているキーコード
    print("キーボード監視を開始します。ウィンドウを閉じるか Ctrl+C で終了します。")
    print("-" * 60)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pressed_keys_set.add(event.key)
                elif event.type == pygame.KEYUP:
                    pressed_keys_set.discard(event.key)
            
            if not running:
                break
            
            # キーボードを描画
            draw_keyboard(screen, pressed_keys_set)
            
            # 押下状態が変わった場合のみ出力
            pressed_keys = sorted(list(pressed_keys_set))
            if pressed_keys != last_pressed_keys:
                ...
                if pressed_keys:
                    ...
                    for key in pressed_keys:
                        ...
                        if vk:
                            vk_codes.append(f"0x{vk:02X}")
                        else:
                            vk_codes.append(f"UNKNOWN_0x{key:X}")
                    print(f"[{timestamp}] 押下キー: {vk_codes}")
                else:
                    print(f"[{timestamp}] キーが押下されていません")
